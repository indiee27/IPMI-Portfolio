...
def demonsReg(source, target, sigma_elastic=1, sigma_fluid=1, num_lev=3, use_composition=False,
  ...
  for lev in range(1, num_lev + 1):
    ...
    if lev == 1:
      ...
      disp_field_x = 2 * rescale(disp_field_x, 2, mode='edge', order=3)
      disp_field_y = 2 * rescale(disp_field_y, 2, mode='edge', order=3)
    ...
    for it in range(max_it):
      ...
      if use_composition:
        ...
        def_field[:, :, 0] = resampImageWithDefField(def_field[:, :, 0], update_def_field)
        def_field[:, :, 1] = resampImageWithDefField(def_field[:, :, 1], update_def_field)
